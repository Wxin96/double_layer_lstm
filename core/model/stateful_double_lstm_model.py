...
def generate_stateful_lstm_model(**kwargs) -> Sequential:
    batch_size = kwargs.get('batch_size', 5)  # 训练样本批次
    time_step = kwargs.get('time_step', 30)  # 时间步长
    input_dim = kwargs.get('input_dim', 3)  # x的特征数目
    ...
    model.add(LSTM(units=hidden_units, activation='tanh',
                   ...
                   stateful=True, batch_input_shape=(batch_size, time_step, input_dim)))
